...
const navGroups = {
  products: [
    { label: "Analytics", path: "/products/analytics", detail: "Simple dropdown destination" },
    { label: "Automation", path: "/products/automation", detail: "Simple dropdown destination" },
    { label: "Scanner API", path: "/products/scanner-api", detail: "Simple dropdown destination" },
  ],
  resources: [
    { label: "Documentation", path: "/resources/docs", detail: "Hover dropdown destination" },
    { label: "Release notes", path: "/resources/releases", detail: "Hover dropdown destination" },
    { label: "Support", path: "/resources/support", detail: "Hover dropdown destination" },
  ],
  solutions: [
    {
      label: "By team",
      children: [
        { label: "Engineering", path: "/solutions/engineering", detail: "Nested submenu destination" },
        { label: "Quality assurance", path: "/solutions/qa", detail: "Nested submenu destination" },
      ],
    },
    {
      label: "By company size",
      children: [
        { label: "Startups", path: "/solutions/startups", detail: "Nested submenu destination" },
        { label: "Enterprise", path: "/solutions/enterprise", detail: "Nested submenu destination" },
      ],
    },
    { label: "All solutions", path: "/solutions", detail: "Direct nested-menu destination" },
  ],
};
...
function Chevron({ right = false }) {
  return <span className={right ? "chevron chevron-right" : "chevron"} aria-hidden="true">⌄</span>;
}
    ...
    <div className="nav-dropdown" ref={rootRef}>
      ...
      <div id="products-menu" className="dropdown-panel" hidden={!open}>
        ...
        {navGroups.products.map((item) => (
          <button key={item.path} type="button" onClick={() => { navigate(item.path, "Click dropdown"); setOpen(false); }}>
...
function HoverDropdown({ navigate }) {
  ...
  return (
    ...
    >
      <button className="nav-trigger" type="button" aria-expanded={open} aria-controls="resources-menu">
        Resources <Chevron />
      </button>
      <div id="resources-menu" className="dropdown-panel" hidden={!open}>
        ...
        {navGroups.resources.map((item) => (
          <button key={item.path} type="button" onClick={() => navigate(item.path, "Hover dropdown")}>
            <span>{item.label}</span><span aria-hidden="true">→</span>
          </button>
        ))}
      </div>
  ...
  return (
    <div className="nav-dropdown">
      ...
      <div className="dropdown-panel nested-panel" hidden={!open}>
        ...
        {navGroups.solutions.map((item) => item.children ? (
          <div className="submenu-root" key={item.label} onMouseEnter={() => setActiveSubmenu(item.label)}>
            <button
              type="button"
              aria-expanded={activeSubmenu === item.label}
              onClick={() => setActiveSubmenu(activeSubmenu === item.label ? "" : item.label)}
            >
              <span>{item.label}</span><Chevron right />
            </button>
            <div className="submenu-panel" hidden={activeSubmenu !== item.label}>
              {item.children.map((child) => (
                <button key={child.path} type="button" onClick={() => { navigate(child.path, "Nested submenu"); setOpen(false); }}>
          ...
          <button key={item.path} type="button" onClick={() => { navigate(item.path, "Nested dropdown"); setOpen(false); }}>
            <span>{item.label}</span><span aria-hidden="true">→</span>
          </button>
        ))}
      </div>
    ...
    <>
      ...
      <aside className={`sidebar ${open ? "open" : ""}`} aria-hidden={!open}>
        <div className="sidebar-heading">
          <div><span className="eyebrow">Alternate navigation</span><h2>Sidebar menu</h2></div>
          <button className="icon-button" type="button" onClick={() => setOpen(false)} aria-label="Close sidebar">×</button>
        </div>
            ...
            <div className="sidebar-links" hidden={expanded !== section.key}>
              {section.items.map((item) => (
                <button
                  ...
                  key={item.path}
                  type="button"
                  onClick={() => { navigate(item.path, "Sidebar dropdown"); setOpen(false); }}
                >
...
function HiddenExample({ item, revealed }) {
  const button = (
    <button className="scanner-target" data-qa-case={item.id} onClick={() => alert(`Activated ${item.label}`)}>
...
function HiddenAttributesLab() {
  ...
  const toggle = (id) => setRevealed((current) => current.includes(id) ? current.filter((value) => value !== id) : [...current, id]);
  return (
    <section className="lab-section" id="hidden-attributes">
      <div className="section-intro">
        <div>
          ...
          <p>Each card contains a real interactive target under a different hidden-state condition.</p>
        </div>
        <button className="secondary-button" type="button" onClick={() => setRevealed(revealed.length === hiddenCases.length ? [] : hiddenCases.map((item) => item.id))}>
          {revealed.length === hiddenCases.length ? "Reset all cases" : "Reveal all targets"}
        </button>
      </div>
        ...
        {hiddenCases.map((item, index) => {
          ...
          return (
            <article className="case-card" key={item.id} data-test-case={item.id}>
              <div className="case-topline"><span>CASE {String(index + 1).padStart(2, "0")}</span><span className={isRevealed ? "status revealed" : "status"}>{isRevealed ? "Revealed" : "Hidden"}</span></div>
              <h3>{item.label}</h3>
              <code>{item.code}</code>
              <HiddenExample item={item} revealed={isRevealed} />
              <button className="case-toggle" type="button" onClick={() => toggle(item.id)}>
                {isRevealed ? "Restore hidden state" : "Reveal test target"} <span aria-hidden="true">→</span>
              </button>
            </article>
          );
        })}
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <button className="brand" type="button" onClick={() => navigate("/", "Brand button")} aria-label="QA Navigation Lab home">
          <span className="brand-mark">Q</span>
          <span><strong>QA NAV</strong><small>reproduction lab</small></span>
        </button>
          ...
          <>
            <section className="hero">
              <div className="hero-copy">
                <span className="eyebrow"><i /> deterministic interaction playground</span>
                <h1>Navigation that is<br /><em>made to be tested.</em></h1>
                <p>A purpose-built React site for reproducing scanner findings across click, hover, nested, redirected, and hidden interactive elements.</p>
                <div className="hero-actions">
                  <button className="primary-button" type="button" onClick={() => document.getElementById("hidden-attributes")?.scrollIntoView({ behavior: "smooth" })}>Explore hidden cases <span>↓</span></button>
                  <button className="secondary-button" type="button" onClick={() => setSidebarOpen(true)}>Open sidebar</button>
                </div>
              </div>
              <div className="route-console">
                <div className="console-top"><span><i /> LIVE ROUTE</span><span>REACT SPA</span></div>
                <div className="route-path">{path}</div>
                <h2>{title}</h2>
                <p>This page was reached through: <strong>{source}</strong></p>
                <div className="console-stats">
                  <span><b>4</b> navigation patterns</span>
                  <span><b>8</b> hidden cases</span>
                  <span><b>18</b> route targets</span>
                </div>
              </div>
            </section>

            <section className="pattern-strip" aria-label="Available interaction patterns">
              <article><span>01</span><strong>Click dropdown</strong><p>Button toggles an explicit menu.</p></article>
              <article><span>02</span><strong>Hover dropdown</strong><p>Pointer hover and keyboard focus.</p></article>
              <article><span>03</span><strong>Nested submenu</strong><p>Second-level destination links.</p></article>
              <article><span>04</span><strong>Simple redirect</strong><p>One action, one route change.</p></article>
            </section>
      ...
      {path === "/" && (
        <footer>
          <div><span className="brand-mark small">Q</span><strong>QA Navigation Lab</strong></div>
